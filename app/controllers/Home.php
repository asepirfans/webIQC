    ...
    public function addQcData()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_type']) && $_POST['form_type'] === 'qc_data') {
            ...
            if (!in_array($file_extension, $allowed_extensions)) {
                Flasher::setFlash('error', 'Gagal!', 'Jenis file tidak diizinkan! Hanya file Excel, PDF, dan Word yang diperbolehkan.');
            ...
            $target_dir = "E:\\File_Server\\QC\\$tanggal\\";
            ...
            if (!move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
                Flasher::setFlash('error', 'Gagal!', 'Gagal mengunggah file!');
                header('Location: ' . BASEURL . '/home');
                exit;
            }
            ...
            if ($this->qcModel->addData($tanggal, $vendor, $code_number, $new_file_name, $progres, $person)) {
                ...
                Flasher::setFlash('success', 'Berhasil!', 'Data berhasil ditambahkan!');
                header('Location: ' . BASEURL . '/home');
                exit;
            } else {
                Flasher::setFlash('error', 'Gagal!', 'Gagal menambahkan data!');
                header('Location: ' . BASEURL . '/home');
                exit;
            }
        }
    ...
    public function addVendorData()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_type']) && $_POST['form_type'] === 'vendor_data') {
            ...
            if ($dataVendor) {
                ...
                Flasher::setFlash('error','Gagal!', 'Vendor sudah terdaftar!');
                header('Location: ' . BASEURL . '/home');
                exit;
            } 
